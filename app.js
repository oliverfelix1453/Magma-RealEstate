...
function page4_text_animation() {
  let eanim = "";
  document
    .querySelector(".page4>h1")
    .textContent.split("")
    .forEach(function (dets) {
      eanim += `<span>${dets}</span>`;
      document.querySelector(".page4>h1").innerHTML = eanim;
    });
...
page4_text_animation();

function canvas2_bridges() {
...
function page6_text_animation() {
  ...
  document
    ...
    .forEach(function (dets) {
      eanim += `<span>${dets}</span>`;
      document.querySelector(".page6>h1").innerHTML = eanim;
    });
...
page6_text_animation();

function canvas3_bookpg() {
...
function info_circle() {
  gsap.to(".info-circle", {
    ...
    backgroundColor: `#1037c66d`,
    scale: 3,
  });
}
info_circle();

function loadingAnimPage1h1() {
  gsap.from(".page1 h1", {
    y: 100,
    opacity: 0,
    delay: 0.5,
...
function imageImprover() {
  ...
  const images = document.querySelectorAll("#scroller-in img");
  ...
  scrollingContainer.style.width = totalWidth + "%";
}
imageImprover();
